...
To document (`roxygenize`) and update the package's DESCRIPTION's Imports and Suggests etc.
as well as the `NAMESPACE` using a declarative config file from `dev/config/attachment.config.yml`,
run the following:
...
NOTE: corresponds directly with internal developer friendly `pkg_sys_*()` utilities
in the `R/utils_pkg.R` script's section. If folders change and accessors are warranted
then add one for the use case, i.e. `pkg_sys_config("config.yml")`, `pkg_sys_shiny("testapp")`, 
etc.

Opinionated standardized folders for specific use cases (use as needed only):
